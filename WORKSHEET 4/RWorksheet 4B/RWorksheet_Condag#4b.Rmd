...
b. b. Get the total number of each variations and save it into another object. Save the
...
```{r}
black_variations <- variation_counts %>%
  filter(grepl("Black", variation))

white_variations <- variation_counts %>%
  filter(grepl("White", variation))

combined_variations <- rbind(black_variations, white_variations)

barplot(height = combined_variations$n,
        names.arg = combined_variations$variation,
        beside = TRUE,
        col = c("black", "white"),
        main = "Black and White Variations",
        xlab = "Variations",
        ylab = "Count",
        legend.text = TRUE)
```